...
export function initScheduler(send: Sender, agentId = 'main'): void {
  ...
  // On startup, recover any tasks stuck in 'running' from a previous crash
  const recovered = recoverStuckTasks(agentId);
  if (recovered > 0) {
    logger.warn({ count: recovered, agentId }, 'Recovered stuck tasks from previous crash');
  }
...
async function runDueTasks(): Promise<void> {
  ...
  for (const task of tasks) {
    // Mark as running BEFORE execution to prevent duplicate firing
    ...
    try {
      ...
      // Reset to active so it can retry on next cycle
      const nextRun = computeNextRun(task.schedule);
      updateTaskAfterRun(task.id, nextRun, `FAILED: ${String(err)}`);
      try {
